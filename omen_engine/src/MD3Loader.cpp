...
inline glm::vec3 rationalvec(glm::vec3 vec) {
...
void MD3Loader::getMesh(std::vector<std::unique_ptr<omen::Mesh>> &meshes) {
	...
	for (auto m : m_meshes) {
		...
		for (auto frame : m->frames) {
			...
			for (auto face : m->faces) {
				for (auto index : face.indices) {
					mesh->vertices().push_back(frame.m_vertices.at(index.vertex_index));
					if (!frame.m_normals.empty())
						mesh->normals().push_back(frame.m_normals.at(index.normal_index));
				}
			}
			//frames.push_back(f);
		}
		//
		mesh->setUVs(m_texcoords);
