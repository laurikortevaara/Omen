...
void MeshRenderer::render(omen::Shader* shader) const
{
	...
	pShader->setUniform4fv("MaterialAmbient", 1, value_ptr(ambientColor));
	pShader->setUniform4fv("MaterialDiffuse", 1, value_ptr(diffuseColor));
	pShader->setUniform4fv("MaterialEmissive", 1, value_ptr(emissiveColor));
	pShader->setUniform4fv("MaterialSpecular", 1, value_ptr(specularColor));
...
void MeshRenderer::render2(Shader* shader)
{
...
#endif
	{
		...
		glm::vec4 emissiveolor = m_meshController->mesh()->material()->const_emissiveColor();
		pShader->setUniform4fv("MaterialEmissive", 1, value_ptr(emissiveolor));
