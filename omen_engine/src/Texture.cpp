...
Texture::Texture(const std::string &bitmap_path, omen::texture_type textureTarget) 
...
uint32_t Texture::createTexture()
{
    ...
    // Convert the Omen texture type to opengl texture type
    GLenum textureTarget = GL_TEXTURE_2D;
    if (m_textureTarget == omen::TEXTURE_2D)
        textureTarget = GL_TEXTURE_2D;
    else if(m_textureTarget == omen::TEXTURE_3D)
        textureTarget = GL_TEXTURE_3D;
    else if (m_textureTarget == omen::TEXTURE_CUBE_MAP)
...
**/
uint32_t Texture::read_png_file(const char* file_name)
{
	png_structp png_ptr;
	png_infop info_ptr;
...
**/
uint32_t Texture::read_jpg_file(const char* fileName)
{
	int x, y, btm;
	FILE *f = nullptr;
