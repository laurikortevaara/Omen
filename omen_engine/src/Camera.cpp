	...
	m_focalLength(DEFAULT_FOCA_LENGTH)
{
	...
	//m_view = perspective(m_fov, aspect, m_near, m_far);
	...
	Engine::instance()->signal_engine_update.connect(this,[this](omen::floatprec time, omen::floatprec deltaTime) {
		...
		m_forward = ((m_rotation) * glm::vec4(0, 0, 1, 0));
		...
		m_pos += normalize(cross(m_forward, m_up))*m_velocity.x;
		m_pos += normalize(cross(m_forward, cameraRight))*m_velocity.y;
...
*/
glm::mat4 Camera::inverseRotation() {
	return inverse(m_rotation);
}
