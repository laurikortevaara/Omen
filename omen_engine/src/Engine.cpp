...
int32_t   Engine::blend_mode = 3;
uint32_t  Engine::ShadowBlur = 3;
float     Engine::ShadowFrustumNear = 5.0f;
float     Engine::ShadowFrustumFar = 1500.0f;
float     Engine::LightDistance = 1.0f;
float     Engine::ShadowFrustumSize = 1000.0f;
glm::vec3      Engine::MousePickRay(0, 0, 0);
glm::vec3      Engine::LightPos(0.00000000000001f, -1.0f, 0.00000001f);
...
{
    std::cout << "Omen Engine initialization starts!" << std::endl;
    ...
    std::cout << "Omen Engine initialization has successfully completed!" << std::endl;
}
...
void Engine::ray_cast_mouse()
{
	...
	MousePickRay = glm::vec3(ray_wor.x, ray_wor.y, ray_wor.z);
...
void Engine::update() {
	...
	*/
    ...
    // Poll the GLFW events to allow window closing, minimizing and resizing
    glfwPollEvents();
}
