...
bool Shader::createShader(GLenum shaderType, uint32_t &shader_id, std::string shader_source) {
	...
	//if (shaderType != GL_COMPUTE_SHADER)
	{
		...
		full_source += "uniform glm::vec4 iMouse;\n"; // MouseCoord(x,y), MouseButtons(Left,Right);
		full_source += "uniform glm::vec2 iResolution;\n"; // WindowSize(width,height);
		full_source += "uniform glm::vec3 iMousePickRay;\n"; // WindowSize(width,height);
		full_source += "uniform glm::vec3 iCameraPosition;\n"; // WindowSize(width,height);
		full_source += "uniform float MATH_PI;\n"; // WindowSize(width,height);
		full_source += "uniform glm::vec4 iViewPort;\n"; // Viewport [x,y,z,w];
	}
...
std::string Shader::getShaderSource(uint32_t &shader_id) {
...
bool Shader::readShaderFile(const char*shader_file) {
	...
	if (is) {
		...
		uint32_t vShader, gShader, tcShader, teShader, fShader, cShader;
...
//		return;
	...
	setUniform4fv("iMouse", 1, value_ptr(mousePos));

	glm::vec2 windowSize(Engine::instance()->window()->width(), Engine::instance()->window()->height());
	setUniform2fv("iResolution", 1, value_ptr(windowSize));

	GLint vport[4];
	glGetIntegerv(GL_VIEWPORT, vport);
	setUniform4iv("iViewPort", 1, vport);

	setUniform3fv("iMousePickRay", 1, value_ptr(Engine::MousePickRay));

	glm::vec3 cameraPosition = Engine::instance()->camera()->position();
	cameraPosition.x *= -1.0f;
	setUniform3fv("iCameraPosition", 1, value_ptr(cameraPosition));
...
void Shader::listAttribs()
{
	...
	GLenum type; // type of the variable (float, glm::vec3 or mat4, etc)
	...
	for (i = 0; i < count; i++)
	{
		glGetActiveAttrib(m_shader_program, (uint32_t)i, bufSize, &length, &size, &type, name);
	...
	for (i = 0; i < count; i++)
	{
		glGetActiveUniform(m_shader_program, (uint32_t)i, bufSize, &length, &size, &type, name);
