...
namespace omen {
    ...
    public:
        ...
        int32_t getAttribLocation(const char*attribName);
        ...
        std::string printShaderSource(uint32_t &shader_id);

        std::string getShaderSource(uint32_t &shader_id);

        bool createShader(uint32_t shaderType, uint32_t &shader_id, std::string shader_source);
    };
} // namespace omen
